...
${fix.alternative_solutions?.map(alt => `
### ${alt.approach}
**Pros:** ${alt.pros?.join(', ')}
**Cons:** ${alt.cons?.join(', ')}
`).join('') || 'None provided'}
`;
    ...
    await axios.put(
      `https://api.github.com/repos/${forkOwner}/${forkName}/contents/${fixFileName}`,
      {
        message: `🔧 Add detailed fix for ${fix.vulnerability_type} in ${fix.file_path}`,
